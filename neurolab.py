...
# korzystamy z gotowego podzialu na czesc treningowa i testowa
...
def exercise_one():
    ...
    y_ = tf.one_hot(correct_y, 10)  # zamienia cyfre na wektor o 10 pozycjach, tylko jedna z nich jest inna niz 0 (hot)
...
def exercise_three():
    ...
    w1 = tf.Variable(tf.truncated_normal(shape=[784, 100], mean=0, stddev=0.2))
    b1 = tf.Variable(tf.constant(value=0.1, shape=[100]))
    h1 = tf.nn.relu(tf.matmul(x_, w1) + b1)

    w2 = tf.Variable(tf.truncated_normal(shape=[100, 10], mean=0, stddev=0.2))
    b2 = tf.Variable(tf.constant(value=0.1, shape=[10]))
    y = tf.nn.softmax(tf.matmul(h1, w2) + b2)
...
def exercise_four():
    ...
    model.add(k_layers.Reshape(target_shape=(784, ), input_shape=(28, 28)))  # zmienia ksztalt wejscia z (28, 28) na (784, )
    ...
    model.add(k_layers.Dense(input_dim=(784, ), units=100, activation='relu'))  # pierwsza prawdziwa warstwa neuronow
    ...
    model.add(k_layers.Dense(input_dim=(784, 100), units=10, activation='softmax'))  # wyjsciowa warstwa neuronow
...
def exercise_five():
    ...
    # TODO: odpowiedz, o ile wieksza jest ta siec od wytrenowanej przez nas? 3 500 000 parametrow
    ...
    layer_to_preview = 3  # numer warstwy, ktorej aktywacje podgladamy
    channel_to_preview = 2   # numer kanalu w tejze warstwie
